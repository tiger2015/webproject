...
<body>
	...
	<div id="center">
		...
		<s:else>
			<%-- <s:form action="student/FindMessage" method="post"> --%>
			<label style="color: blue;">选择班级</label>
			<select name="classId" id="chatclassid">
					...
					<s:else>
						<table class="showtable">
							...
							<s:iterator value="#request.messages">
								<tr>
									<td><s:url action="student/FindResponse" var="myurl"
											escapeAmp="true">
									...
									<td><s:property value="userinfo['userId']" />
									</td>
									<td><s:property value="responseCount" />
									</td>
								</tr>
							</s:iterator>
						</table>
						<a
							href="<%=basePath%>/student/FindMessage?classId=<%=classId%>&currentpage=<%=1%>">首页</a>
						<a
							href="<%=basePath%>/student/FindMessage?classId=<%=classId%>&currentpage=<%=splitePage.getCurrentPage() + 1%>">下一页</a>
						<a
							href="<%=basePath%>/student/FindMessage?classId=<%=classId%>&currentpage=<%=splitePage.getCurrentPage() - 1%>">上一页</a>
						<a
							href="<%=basePath%>/student/FindMessage?classId=<%=classId%>&currentpage=<%=splitePage.getTotalPage()%>">尾页</a>
